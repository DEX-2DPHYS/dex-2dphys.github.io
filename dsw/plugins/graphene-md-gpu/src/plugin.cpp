...
struct Instance {
    ...
    // ---- GPU backend. "auto" tries the GPU and falls back silently;
    // "cpu"/"gpu" force the choice ("gpu" still falls back if there is none,
    // but reports why). gpuLive means device buffers currently hold the truth.
    clmd::GpuMD gpu;
    std::string backend = "auto";
    bool gpuLive = false;
    int gpuTopoVersion = -1;
    double gpuMs = 0;   // measured ms per step on the current backend
    long long frame = 0;
...
#pragma omp parallel for schedule(static)
    ...
    bool wantGpu() {
        ...
        if (!gpu.ready && !gpu.init()) return false;
        return true;
    }

    // Fill the kernel parameter block from P (indices documented in clmd.h).
    void gpuParams(double gasF) {
        float *par = gpu.par;
        par[0] = (float)P.De; par[1] = (float)P.alpha; par[2] = (float)P.re;
        par[3] = (float)P.ktheta; par[4] = (float)(P.theta0 * M_PI / 180);
        par[5] = (float)P.kbend;
        par[6] = (float)(P.sigma * P.sigma);
        par[7] = (float)(P.eps0 * P.escale);
        par[8] = (float)(9 * P.sigma * P.sigma);
        par[9] = (float)P.edgeK;
        par[10] = (float)gasF;
        par[11] = (float)(P.Cxnm * NM); par[12] = (float)(P.Cynm * NM);
        par[13] = (float)(P.Mxnm * NM / 2); par[14] = (float)(P.Mynm * NM / 2);
        par[15] = (float)(P.Mxnm * NM / 3); par[16] = (float)(P.Mynm * NM / 3);
        par[17] = (float)(P.bubbleRnm * NM * P.bubbleRnm * NM);
        par[18] = P.profile == "mesa" ? 1.0f : (P.profile == "bubble" ? 2.0f : 0.0f);
        par[19] = (float)std::exp(-(P.gamma * 1000) * P.dtFs / 2000);
        par[20] = (float)P.dtFs;
        par[21] = (float)P.maxV;
        par[22] = (float)(P.maxDX * (P.dtFs / 0.5));
        par[23] = (float)ACC_K;
        par[24] = (P.edgeK >= 200) ? 1.0f : 0.0f;
        par[29] = (float)elevz;
        gpu.pushPar();
    }

    // Same gas-force scalar the CPU force loop derives.
    double gasForceNow() const {
        if (P.protLoc != "between") return 0;
        const bool bubbleProfile = (P.profile == "bubble");
        const double target = bubbleProfile ? henckyH0() : P.targetDz;
        const double frac = target > 0 ? clampd(elevz / target, 0, 1) : 0;
        if (frac <= 0) return 0;
        const double boost = P.betweenBoost > 0 ? P.betweenBoost : 1;
        return (P.bubbleP * 1e6 * frac) * PA_TO_EV_A3 * A_ATOM * boost;
    }

    void gpuEnter() {
        gpu.uploadState(*this);
        if (subDirty) buildSubCells();
        gpu.uploadSubstrate(*this);
        gpu.uploadTopology(*this);
        gpuTopoVersion = (int)topoVersion;
        gpuLive = true;
        haveForces = false;
    }
    void gpuLeave() {
        if (!gpuLive) return;
        gpu.downloadState(*this);
        gpuLive = false;
        haveForces = false;
    }

    // One advance-batch of k steps, entirely on the device. Bond breaking /
    // re-forming runs once per batch on the CPU (positions come back anyway
    // for the frame): a bond takes many steps to stretch to 1.2 re, so the
    // per-batch cadence is not observable.
    void gpuBatch(int k) {
        const size_t n = nTop();
        if (!n) return;
        if (!gpuLive) gpuEnter();
        if ((int)topoVersion != gpuTopoVersion) {
            gpu.uploadTopology(*this);
            gpuTopoVersion = (int)topoVersion;
        }
        ...
        for (int st = 0; st < k; st++) {
            ...
            const bool subMoved = stepElevation();   // scalar state machine
            gpuParams(gasForceNow());
            if (subMoved && P.protLoc != "between") gpu.lift((int)nSub());
            if (!haveForces || subMoved) {
                gpu.bend((float)P.kbend, (int)n);
                gpu.forces((int)n);
                haveForces = true;
            }
            gpu.kick(0, (int)n);
            ...
            gpu.bend((float)P.kbend, (int)n);
            gpu.forces((int)n);
            gpu.kick(st == k - 1 ? 1 : 0, (int)n);
        }
        // bring back what the CPU-side machinery needs
        gpu.downF(gpu.dTx, tx, n); gpu.downF(gpu.dTy, ty, n); gpu.downF(gpu.dTz, tz, n);
        std::vector<double> v2, peA;
        gpu.downF(gpu.dV2, v2, n);
        gpu.downF(gpu.dPe, peA, n);
        double ke = 0, pe = 0;
        for (size_t i = 0; i < n; i++) { ke += v2[i]; pe += peA[i]; }
        eKin = 0.5 * C_MASS * AMU_A2_FS2_IN_EV * ke;
        temperature = n ? (2.0 * eKin) / (3.0 * (double)n * KB_EV) : 0;
        ePot = pe;
        // substrate z changed on-device during a lift; mirror it for display
        if (elevActive && P.protLoc != "between") {
            gpu.downF(gpu.dSz, sz, nSub());
            subVersion++;
        }
        clmd::api().Finish(gpu.q);
        gpuMs = std::chrono::duration<double>(
                    std::chrono::steady_clock::now() - t0).count() * 1000.0 / k;

        // Topology maintenance on fresh positions. If bonds changed, the
        // velocities on the device stay valid; only the CSR tables reload.
        const uint64_t before = topoVersion;
        updateTopology();
        if (topoVersion != before) {
            gpu.uploadTopology(*this);
            gpuTopoVersion = (int)topoVersion;
            haveForces = false;
        }
        ljValid = false;   // CPU lists are stale; rebuilt lazily if needed
    }

    // Advance only the elevation scalar state (the part of applyProtrusion
    // that is not per-atom). Returns true when the substrate profile moved.
    bool stepElevation() {
        if (!elevActive) return false;
        const double target = (P.profile == "bubble") ? henckyH0() : P.targetDz;
        if (P.elevMode == "const") {
...
#pragma omp parallel for schedule(static) reduction(+ : ke)
    ...
    void computeRegistry() {
        if (P.regHeightDamp && ljNeedsRebuild()) buildLJLists();
        const size_t n = nTop();
        regT.assign(n, 0.0f);
        regW.assign(n, 1.0f);
...
#pragma omp parallel for schedule(static)
    ...
    void sayState(long q) {
        ...
        snprintf(buf, sizeof buf,
                 ...
                 1,
#endif
                 gpu.ready ? gpu.device.c_str()
                           : (gpu.error.empty() ? "untried" : gpu.error.c_str()),
                 (backend != "cpu" && gpu.ready) ? 1 : 0
        );
        say(buf);
    }
    ...
    void readParams(const std::string &m) {
        ...
        {
            std::string b = dexmsg::get_str(m, "backend");
            if (b == "auto" || b == "cpu" || b == "gpu") backend = b;
        }
    ...
    // Continue-from-now snapshot -> runnable LAMMPS deck (classic engine
    // only in this bundle). handle() has already pulled state off the GPU.
    void exportLmpDeck() {
        lmpexport::Deck D;
        D.lammpsEngine = false;
        D.materialKey = "graphene";
        D.nsp = 1; D.mass[0] = C_MASS;
        ...
        // fix viscous coefficient: m * gamma[1/ps] in metal units
        // (1 amu*A^2/ps^2 = 1.03642e-4 eV)
        D.viscous = C_MASS * (P.gamma * 1000.0) * 1.03642e-4;
        D.Nnm = P.Nnm;
        ...
        D.henckyH0 = henckyH0(); D.areaAtom = A_ATOM;
        ...
        for (size_t i = 0; i < n; i++) {   // A/fs -> A/ps
            D.vx[i] = vx[i] * 1000.0;
            D.vy[i] = vy[i] * 1000.0;
            D.vz[i] = vz[i] * 1000.0;
        }
    ...
    void handle(const std::string &m) {
        // Any message may touch positions, topology or geometry on the CPU
        // side (build, elev, params, poke...). Cheapest correct rule: leave
        // the device before handling, re-enter lazily on the next batch.
        gpuLeave();
...
int advance(void *p, double dt) {
    ...
    int done = 0;
    if (s->wantGpu()) {
        // The whole batch runs device-side; one transfer pair per batch.
        s->gpuBatch(budget);
        done = budget;
        s->stepMs = s->gpuMs;
    } else {
        s->gpuLeave();   // pull state back if we were on the device
        bool cutShort = false;
        ...
        // Budget was taken in full, so hand back whatever the 10 ms cap cut
        // short; otherwise an explicit "step n" is silently truncated. (The
        // device path above has no such cap -- it runs the whole batch.)
