...
// LAMMPS C library interface (built as liblammps.a, MANYBODY package): the
// real many-body potentials — AIREBO for graphene, ExTeP for hBN, REBO-MoS2
// for MoS2 — run inside an embedded LAMMPS instance. The parameter files live
// in the bundle's potentials/ folder, straight from lammps/potentials.
#include "library.h"
#include <winsock2.h>
#include <ws2tcpip.h>
#include "lmpexport.h"
...
// Folder this DLL sits in — the bundle root, so <dir>/potentials/<file>.
std::string bundleDir() {
    ...
    GetModuleHandleExA(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS |
                       ...
                       (LPCSTR)&bundleDir, &hm);
    char buf[1024] = {0};
    GetModuleFileNameA(hm, buf, sizeof buf - 1);
    std::string p(buf);
    for (auto &c : p) if (c == '\\') c = '/';  // LAMMPS commands take '/'
#else
    Dl_info di{};
    dladdr((void *)&bundleDir, &di);
...
// 1 amu·Å²/ps² in eV, for kinetic energy from LAMMPS metal-unit velocities.
constexpr double AMU_A2_PS2_IN_EV = 1.03642696e-4;

// ------------------------------------------------------------- materials
//
// One entry per 2D material. The classic Morse engine only makes sense for
// graphene; hBN and MoS2 always run on the embedded LAMMPS potentials.
// Interlayer LJ (sheet <-> rigid substrate) stays the plugin's own, with
// per-material defaults; E2D feeds the Hencky blister height.

struct MatSpec {
    const char *key;         // wire name
    int nsp;                 // species count (3 = M + top X + bottom X)
    double mass[3];          // amu, LAMMPS type order
    double aLatt;            // in-plane lattice constant, Å
    double zS;               // trilayers: chalcogen-plane half-thickness, Å
    double z0;               // default interlayer rest height, Å
    double sigma, eps0;      // interlayer LJ defaults (Å, eV)
    double bondCut;          // render-bond distance cutoff, Å
    double E2Dnm;            // 2D modulus, N/m (Hencky)
    double areaAtom;         // footprint per sheet atom, Å² (gas pressure)
    bool trilayer;           // X-M-X sandwich
    bool bondCross;          // bonds only metal<->non-metal (or A<->B)
    const char *pairStyle;   // LAMMPS pair_style command
    const char *potFile;     // file in potentials/
    const char *coeffTail;   // element mapping after the filename
};

// Trilayer z0 defaults are 2*zS + 2^(1/6)*sigma: the interlayer X–X vdW gap
// starts AT the LJ minimum (starting inside the repulsive wall tears the
// sheet apart — measured the hard way on MoS2).
// mos2 zS 1.62: rebomos's own relaxed S-plane height on the correct lattice
// (crystallographic 1.564 is only ~26 meV/atom off — gentle).
// ws2/mote2/wte2: reduced 3-type Stillinger-Weber from Jiang & Zhou
// (arXiv:1704.03147), see potentials/*.jiang3t.sw; interlayer LJ for Te is
// UFF (sigma 3.98 Å, eps 17.3 meV).
const MatSpec MATS[] = {
    {"graphene", 1, {12.011, 0, 0}, 2.46, 0, 3.35,
     3.42, 2.387e-3, 1.85, 340, 2.62, false, false,
     "pair_style airebo 3.0", "CH.airebo", "C"},
    {"hbn", 2, {10.811, 14.007, 0}, 2.504, 0, 3.33,
     3.33, 2.6e-3, 1.95, 289, 2.71, false, true,
     "pair_style extep", "BN.extep", "B N"},
    {"mos2", 2, {95.95, 32.06, 0}, 3.16, 1.62, 6.75,
     3.13, 6.93e-3, 2.90, 130, 2.88, true, true,
     "pair_style rebomos", "MoS.rebomos", "Mo S"},
    {"ws2", 3, {183.84, 32.06, 32.06}, 3.13, 1.564, 6.64,
     3.13, 6.93e-3, 3.00, 121.5, 2.83, true, true,
     "pair_style sw", "ws2.jiang3t.sw", "W S1 S2"},
    {"mote2", 3, {95.95, 127.60, 127.60}, 3.55, 1.803, 8.07,
     3.98, 17.3e-3, 3.30, 79.8, 3.64, true, true,
     "pair_style sw", "mote2.jiang3t.sw", "Mo Te1 Te2"},
    {"wte2", 3, {183.84, 127.60, 127.60}, 3.55, 1.803, 8.07,
     3.98, 17.3e-3, 3.30, 82.7, 3.64, true, true,
     "pair_style sw", "wte2.jiang3t.sw", "W Te1 Te2"},
};

const MatSpec *matByKey(const std::string &k) {
...
struct Params {
    ...
    // material + engine (rebuild). engine: "classic" (Morse toy model,
    // graphene only) or "lammps" (AIREBO / ExTeP / REBO-MoS2).
    std::string material = "graphene", engine = "classic";
    // physics
    ...
    double gasT = 300;        // gas temperature for the fixed-N blister, K
    double gasGap = 1.2;      // A of standoff that counts as delaminated
                              // ("bubbleFree"); ~one molecular diameter
...
struct Instance {
    ...
    // Species per atom (index into mat->sym), sheet and substrate.
    std::vector<uint8_t> tSpec, sSpec;
    const MatSpec *mat = &MATS[0];

    // Embedded LAMMPS engine.
    void *lmp = nullptr;         // LAMMPS handle, or null
    bool lmpActive = false;      // engine running this build
    bool lmpDirty = false;       // live params changed since last run
    int64_t lmpLastTs = -1;      // dedupe fix-external callbacks
    long bondScanClock = 0;      // render-bond rescan cadence
    std::string lmpError;        // last engine failure, for the UI
    ...
    // Fixed-N gas blister ("bubbleN"): molecules in the pocket, the measured
    // pocket volume and the resulting pressure. Reported in the state message
    // so the panel can watch the gas law work.
    double gasN = 0, gasVA3 = 0, gasPa = 0, gasRA = 0;
    ...
    double temperatureAll = 0;   // including evaporated atoms (the old meaning)
    int nEvaporated = 0;         // atoms rejected from `temperature`
    std::vector<double> speedScratch;   // reused by evapTemperature()
    double stepMs = 0;
    ...
    // Square patch of the current material, |x|,|y| <= halfA, sheet mid-plane
    // at height z. Honeycombs put sublattice A/B at species 0/1 (B/N for hBN,
    // both C for graphene); the MoS2 trilayer is Mo at z with an S plane at
    // z +- zS, S on the (2/3, 1/3) site.
    void genLattice(double halfA, double z, std::vector<double> &X,
                    std::vector<double> &Y, std::vector<double> &Z,
                    std::vector<uint8_t> &SP) const {
        X.clear(); Y.clear(); Z.clear(); SP.clear();
        const double a = mat->aLatt;
        ...
        for (int i = -nx; i <= nx; i++)
            for (int j = -ny; j <= ny; j++) {
                const double px = i * a1x + j * a2x, py = i * a1y + j * a2y;
                if (mat->trilayer) {
                    // M at the cell origin; the eclipsed X pair sits on the
                    // (1/3,1/3) hollow, (a1+a2)/3 — in-plane a/√3 from M so
                    // the M–X bond length is right. NOT (a1+2·a2)/3: that
                    // site is 2.8 Å out and no potential recognises a bond
                    // (cost a day of "why is everything soft/unstable").
                    const double bx = px + (a1x + a2x) / 3;
                    const double by = py + (a1y + a2y) / 3;
                    if (std::fabs(px) <= halfA && std::fabs(py) <= halfA) {
                        X.push_back(px); Y.push_back(py); Z.push_back(z);
                        SP.push_back(0);
                    }
                    if (std::fabs(bx) <= halfA && std::fabs(by) <= halfA) {
                        // 3-species potentials distinguish the two planes.
                        const uint8_t st = 1, sb = mat->nsp > 2 ? 2 : 1;
                        X.push_back(bx); Y.push_back(by); Z.push_back(z + mat->zS); SP.push_back(st);
                        X.push_back(bx); Y.push_back(by); Z.push_back(z - mat->zS); SP.push_back(sb);
                    }
                } else {
                    const double bx[2] = {0, a / 2};
                    const double by[2] = {0, std::sqrt(3.0) * a / 6};
                    for (int b = 0; b < 2; b++) {
                        const double x = px + bx[b], y = py + by[b];
                        if (std::fabs(x) <= halfA && std::fabs(y) <= halfA) {
                            X.push_back(x); Y.push_back(y); Z.push_back(z);
                            SP.push_back(mat->nsp > 1 ? (uint8_t)b : 0);
                        }
                    }
                }
            }
    ...
    // hBN and MoS2 have no Morse parameterization here: they always run on
    // the LAMMPS potentials. Graphene runs on either engine.
    bool useLmp() const { return P.engine == "lammps" || mat != &MATS[0]; }

    void build() {
        mat = matByKey(P.material);
        const double halfSub = P.Nsubnm * NM / 2, halfTop = P.Nnm * NM / 2;
        genLattice(halfSub, 0, sx, sy, sz, sSpec);
        ...
        genLattice(halfTop * std::sqrt(2.0) + 3, P.z0, rx, ry, rz, rs);
        const double th = P.twistDeg * M_PI / 180, ct = std::cos(th), st = std::sin(th);
        tx.clear(); ty.clear(); tz.clear(); tSpec.clear();
        ...
        buildSubCells();
        if (useLmp()) {
            bondScanLmp();      // render bonds by distance; LAMMPS owns forces
        } else {
            rebuildConnectivity();
        }
        ljValid = false;
        ...
        if (useLmp()) lmpStart(); else lmpStop();
    }
...
#pragma omp parallel for schedule(static)
    ...
    // Volume of the gas pocket, measured from the sheet itself: the height of
    // every atom over the substrate contact plane, inside the blister
    // footprint, times the area each atom stands for. Cubic angstroms.
    // True where the gas is in contact with the sheet. For "bubbleFree" that is
    // the seed disc plus anything the gas has peeled open; otherwise it is the
    // fixed footprint and nothing else.
    inline bool gasWets(size_t i, double R2, double cx, double cy, bool freeFront) const {
        const double dx = tx[i] - cx, dy = ty[i] - cy;
        if (dx * dx + dy * dy < R2) return true;          // seed / fixed footprint
        return freeFront && (tz[i] - P.z0) > P.gasGap;    // peeled open
    }

    double blisterVolume() const {
        const double R2 = P.bubbleRnm * NM * P.bubbleRnm * NM;
        const double cx = P.Cxnm * NM, cy = P.Cynm * NM;
        const double A = mat->areaAtom;
        const bool freeFront = (P.profile == "bubbleFree");
        double V = 0;
        const int n = (int)nTop();
#ifdef _OPENMP
#pragma omp parallel for schedule(static) reduction(+ : V)
#endif
        for (int ii = 0; ii < n; ii++) {
            const size_t i = (size_t)ii;
            if (!gasWets(i, R2, cx, cy, freeFront)) continue;
            const double h = tz[i] - P.z0;
            if (h > 0) V += h * A;
        }
        // Zero volume would mean infinite pressure. Floor it at a half-angstrom
        // gap over the footprint: below that the "gas" is not a gas at all,
        // it is two surfaces in contact.
        const double Vmin = 3.14159265358979 * R2 * 0.5;
        return V > Vmin ? V : Vmin;
    }

    // Radius of the wetted (delaminated) region -- the quantity the blister test
    // actually differentiates with respect to, and an OUTPUT under "bubbleFree".
    double wettedRadius() const {
        const double R2 = P.bubbleRnm * NM * P.bubbleRnm * NM;
        const double cx = P.Cxnm * NM, cy = P.Cynm * NM;
        const bool freeFront = (P.profile == "bubbleFree");
        double r2max = 0;
        const int n = (int)nTop();
#ifdef _OPENMP
#pragma omp parallel for schedule(static) reduction(max : r2max)
#endif
        for (int ii = 0; ii < n; ii++) {
            const size_t i = (size_t)ii;
            if (!gasWets(i, R2, cx, cy, freeFront)) continue;
    ...
    // Molecules needed to reach the requested pressure at the Hencky
    // equilibrium. Fixing N this way makes the blister settle AT the Hencky
    // height by construction instead of blowing straight past it.
    double gasNTarget() const {
        const double h0 = henckyH0();
        const double R = P.bubbleRnm * NM;
        if (!(h0 > 0) || !(R > 0)) return 0;
        // Hencky cap volume: integral of h0 (1-rho^2)^(2/3) dA = (3/5) pi h0 R^2
        const double V_m3 = 0.6 * 3.14159265358979 * h0 * R * R * 1e-30;
        const double kT = 1.380649e-23 * (P.gasT > 0 ? P.gasT : 300.0);
        return (P.bubbleP * 1e6) * V_m3 / kT;
    }
    ...
    double henckyH0() const {
        ...
        return 0.709 * R_m * std::cbrt(p_Pa * R_m / mat->E2Dnm) * 1e10;  // Å
    }

    // ------------------------------------------------ ML sidecar client
    //
    // engine "ml": the classic integrator keeps the substrate LJ, collar and
    // gas terms, but the IN-PLANE forces come from a Python sidecar running a
    // real ML potential (fairchem/UMA) — or a labelled surrogate for pipeline
    // tests. One newline-delimited JSON round trip per force evaluation.
    // Failure is graceful: notice + fall back to the classic engine.

    SOCKET mlSock = INVALID_SOCKET;
    std::string mlModel, mlError;
    double mlMs = 0;

    bool mlConnect() {
        if (mlSock != INVALID_SOCKET) return true;
        static bool wsaDone = false;
        if (!wsaDone) { WSADATA w; WSAStartup(MAKEWORD(2, 2), &w); wsaDone = true; }
        SOCKET so = socket(AF_INET, SOCK_STREAM, 0);
        sockaddr_in a{};
        a.sin_family = AF_INET;
        a.sin_port = htons(8977);
        inet_pton(AF_INET, "127.0.0.1", &a.sin_addr);
        if (connect(so, (sockaddr *)&a, sizeof a) != 0) {
            closesocket(so);
            mlError = "no sidecar on 127.0.0.1:8977 (start ml/start-ml-server.bat)";
            return false;
        }
        DWORD tmo = 600000;   // a big model on CPU can take minutes per call
        setsockopt(so, SOL_SOCKET, SO_RCVTIMEO, (const char *)&tmo, sizeof tmo);
        mlSock = so;
        std::string reply;
        if (!mlRoundTrip("{\"cmd\":\"hello\"}\n", reply)) return false;
        const size_t k = reply.find("\"model\":\"");
        if (k != std::string::npos) {
            const size_t b = k + 9, e2 = reply.find('"', b);
            mlModel = reply.substr(b, e2 - b);
        }
        mlError.clear();
        return true;
    }
    void mlDisconnect() {
        if (mlSock != INVALID_SOCKET) { closesocket(mlSock); mlSock = INVALID_SOCKET; }
    }
    bool mlRoundTrip(const std::string &req, std::string &reply) {
        if (send(mlSock, req.data(), (int)req.size(), 0) != (int)req.size()) {
            mlDisconnect(); mlError = "sidecar send failed"; return false;
        }
        reply.clear();
        char buf[1 << 16];
        for (;;) {
            const int r = recv(mlSock, buf, sizeof buf, 0);
            if (r <= 0) { mlDisconnect(); mlError = "sidecar closed / timed out"; return false; }
            reply.append(buf, (size_t)r);
            if (!reply.empty() && reply.back() == '\n') return true;
        }
    }

    // Ask the sidecar for in-plane forces at the current sheet positions.
    // Adds into fx/fy/fz and returns the model energy, or NaN on failure.
    double mlForces() {
        const size_t n = nTop();
        if (!mlConnect()) return NAN;
        const auto t0 = std::chrono::steady_clock::now();
        std::string req;
        req.reserve(24 * 3 * n + 64);
        req += "{\"cmd\":\"forces\",\"pos\":[";
        char b[64];
        for (size_t i = 0; i < n; i++) {
            snprintf(b, sizeof b, "%s%.6f,%.6f,%.6f", i ? "," : "", tx[i], ty[i], tz[i]);
            req += b;
        }
        req += "]}\n";
        std::string rep;
        if (!mlRoundTrip(req, rep)) return NAN;
        const size_t fk = rep.find("\"f\":[");
        if (rep.find("\"ok\":1") == std::string::npos || fk == std::string::npos) {
            mlError = "sidecar error reply"; return NAN;
        }
        const double e = dexmsg::get_num(rep, "e", NAN);
        const char *c = rep.c_str() + fk + 5;
        for (size_t i = 0; i < n; i++) {
            char *end;
            const double gx = strtod(c, &end); c = end + 1;
            const double gy = strtod(c, &end); c = end + 1;
            const double gz = strtod(c, &end); c = end;
            while (*c == ',' || *c == ' ') c++;
            fx[i] += gx; fy[i] += gy; fz[i] += gz;
        }
        mlMs = std::chrono::duration<double>(
...
#pragma omp parallel for schedule(static)
        ...
        const bool freeFront = (P.profile == "bubbleFree");
        const bool gasLaw = (P.profile == "bubbleN") || freeFront;
        const bool bubbleProfile = (P.profile == "bubble" || gasLaw);
        if (P.protLoc == "between") {
            ...
            if (gasLaw) {
                // Fixed-N: the ramp pumps molecules in and out, and the pressure
                // is whatever the gas law gives for the volume the sheet is
                // currently enclosing. No betweenBoost - see gasNTarget().
                gasN = gasNTarget() * frac;
                gasVA3 = blisterVolume();
                gasRA = wettedRadius();
                const double kT = 1.380649e-23 * (P.gasT > 0 ? P.gasT : 300.0);
                gasPa = gasN > 0 ? gasN * kT / (gasVA3 * 1e-30) : 0.0;
                gasF = gasPa * PA_TO_EV_A3 * mat->areaAtom;
            } else if (frac > 0) {
                const double boost = P.betweenBoost > 0 ? P.betweenBoost : 1;
                gasF = (P.bubbleP * 1e6 * frac) * PA_TO_EV_A3 * mat->areaAtom * boost;
            }
        }
...
#pragma omp parallel for schedule(static) reduction(+ : pe)
        ...
        for (int ii = 0; ii < (int)n; ii++) {
            ...
            // (under the ML engine the sidecar owns ALL in-plane physics)
            const int bs = nbOff[i], be = nbOff[i + 1];
            for (int k = ml ? be : bs; k < be; k++) {
                const int j = nbIdx[k];
            ...
            if (gasF > 0) {
                ...
                if (bubbleProfile)
                    w = ((dx * dx + dy * dy) < bubR2 ||
                         (freeFront && (zi - P.z0) > P.gasGap)) ? 1 : 0;
        ...
        ePot = pe;
        if (ml) {
            const double eMl = mlForces();   // adds into fx/fy/fz
            if (eMl != eMl) {                // NaN: sidecar gone — fall back
                std::string e2 = mlError;
                for (auto &ch : e2) if (ch == '"' || ch == '\\') ch = ' ';
                say("{\"t\":\"notice\",\"msg\":\"ML sidecar: " + e2 +
                    " - falling back to classic engine\"}");
                P.engine = "classic";
                computeForces();             // recompute honestly, once
                return;
            }
            ePot += eMl;
        }
    ...
    bool applyProtrusion() {
        ...
        const double target = (P.profile == "bubble" || P.profile == "bubbleN")
                                  ? henckyH0() : P.targetDz;
        if (P.elevMode == "const") {
...
#pragma omp parallel for schedule(static) reduction(+ : ke)
        ...
        {
            std::vector<double> s2(n), ms(n, C_MASS);
            for (size_t i = 0; i < n; i++)
                s2[i] = vx[i] * vx[i] + vy[i] * vy[i] + vz[i] * vz[i];
            setTemperatures(s2, ms, AMU_A2_FS2_IN_EV);
        }
        frame++;
        stepsDone++;
        stepsWindow++;
    ...
    // Temperature of the SHEET, with evaporated atoms rejected.
    //
    // A few atoms that have broken free travel far faster than anything still
    // bonded, and kinetic energy goes as v^2, so they dominate the average
    // completely: measured 8 atoms out of 5481 carrying 79 % of the total KE,
    // giving 1048 K where the sheet was at 219 K. Reject on the MEDIAN speed
    // (mean and sd are themselves wrecked by the outliers we are looking for).
    // Sets temperature / temperatureAll / nEvaporated from per-atom speeds
    // squared in `s2` and the matching per-atom masses in amu.
    void setTemperatures(const std::vector<double> &s2,
                         const std::vector<double> &massAmu, double unitToEv) {
        const size_t n = s2.size();
        nEvaporated = 0;
        if (!n) { temperature = temperatureAll = 0; return; }
        double keAll = 0;
        for (size_t i = 0; i < n; i++) keAll += 0.5 * massAmu[i] * s2[i];
        keAll *= unitToEv;
        eKin = keAll;
        temperatureAll = (2.0 * keAll) / (3.0 * (double)n * KB_EV);

        speedScratch.assign(s2.begin(), s2.end());
        const size_t mid = n / 2;
        std::nth_element(speedScratch.begin(), speedScratch.begin() + mid,
                         speedScratch.end());
        const double med = speedScratch[mid];
        // Everything slower than 25x the median speed-squared (5x the median
        // speed) counts as part of the sheet. Bonded thermal motion never gets
        // near that; an evaporated atom is far past it.
        const double cut = med * 25.0;
        double ke = 0;
        size_t kept = 0;
        for (size_t i = 0; i < n; i++) {
            if (s2[i] <= cut || med <= 0) { ke += 0.5 * massAmu[i] * s2[i]; kept++; }
            else nEvaporated++;
        }
        ke *= unitToEv;
        temperature = kept ? (2.0 * ke) / (3.0 * (double)kept * KB_EV) : 0;
    }
    ...
    void computeRegistry() {
        ...
        regT.assign(n, 0.0f);
        regW.assign(n, 1.0f);
        const double g = 4 * M_PI / (std::sqrt(3.0) * mat->aLatt);
        double Gx[3], Gy[3];
        for (int k = 0; k < 3; k++) {
...
#pragma omp parallel for schedule(static)
    ...
    // ------------------------------------------------------ LAMMPS engine
    //
    // The sheet's interatomic forces come from an embedded serial LAMMPS
    // instance (units metal, shrink-wrapped box, fix nve + fix viscous).
    // Everything the plugin adds on top — the rigid substrate's LJ, the gas
    // pocket, the soft edge collar, the protrusion kinematics — is injected
    // through fix external's per-step callback, so the two force sources
    // combine inside LAMMPS's own velocity-Verlet update. The plugin arrays
    // tx/ty/tz shadow the LAMMPS positions (copied in the callback), so the
    // whole render/registry/protocol path is engine-agnostic.
    ...
    bool lmpCmd(const char *fmt, ...) {
        ...
        if (lammps_has_error(lmp)) {
            char err[512] = {0};
            lammps_get_last_error_message(lmp, err, sizeof err - 1);
            lmpError = std::string(err) + " [" + buf + "]";
            return false;
        }
        return true;
    }

    // Map P.gamma (the classic engine's per-fs damping rate) onto a
    // fix viscous coefficient, F = -gv * v with v in Å/ps.
    double viscousCoeff() const {
        double mavg = 0;
        for (int s = 0; s < mat->nsp; s++) mavg += mat->mass[s];
        mavg /= mat->nsp;
        return mavg * (P.gamma * 1000.0) * AMU_A2_PS2_IN_EV;
    }

    // Substrate LJ + soft edge collar + gas pocket, evaluated at the LAMMPS
    // positions and written into fix external's force array. Returns the
    // potential energy of what it added.
    double computeExtForces(int nlocal, const int32_t *ids, double *const *x,
                            double *const *fext) {
        // Shadow the LAMMPS positions first: every consumer below (LJ lists,
        // registry, render) reads tx/ty/tz.
        for (int li = 0; li < nlocal; li++) {
            const int i = ids[li] - 1;
            tx[i] = x[li][0]; ty[i] = x[li][1]; tz[i] = x[li][2];
        }
        if (ljNeedsRebuild()) buildLJLists();

        const double sigma = P.sigma, eps = P.eps0 * P.escale;
        const double ljCut2 = 9 * sigma * sigma, sig2 = sigma * sigma;
        const double edgeK = (P.edgeK > 0 && P.edgeK < 200) ? P.edgeK : 0;
...
#pragma omp parallel for schedule(static) reduction(+ : pe)
    ...
    static void extCallback(void *ptr, int64_t ts, int nlocal, int32_t *ids,
        ...
        // One protrusion tick per MD timestep, however often LAMMPS calls in.
        if (ts != s->lmpLastTs) {
            s->lmpLastTs = ts;
            if (s->applyProtrusion()) s->ljValid = false;
        }
        const double e = s->computeExtForces(nlocal, ids, x, fext);
        lammps_fix_external_set_energy_global(s->lmp, "ext", e);
    }

    bool lmpStartInner() {
        const char *argv[] = {"gmd", "-screen", "none", "-log", "none", "-nocite"};
        lmp = lammps_open_no_mpi(6, (char **)argv, nullptr);
        if (!lmp) { lmpError = "lammps_open_no_mpi failed"; return false; }
        const size_t n = nTop();
        if (!n) { lmpError = "empty sheet"; return false; }

        double lo[3] = {1e30, 1e30, 1e30}, hi[3] = {-1e30, -1e30, -1e30};
        for (size_t i = 0; i < n; i++) {
            lo[0] = std::min(lo[0], tx[i]); hi[0] = std::max(hi[0], tx[i]);
            lo[1] = std::min(lo[1], ty[i]); hi[1] = std::max(hi[1], ty[i]);
            lo[2] = std::min(lo[2], tz[i]); hi[2] = std::max(hi[2], tz[i]);
        }
        const double zTop = hi[2] + std::max(60.0, P.targetDz + 40.0);

        if (!lmpCmd("units metal")) return false;
        if (!lmpCmd("dimension 3")) return false;
        // Thread the pair styles (airebo/omp etc.). Styles without an /omp
        // variant (extep, rebomos) silently fall back to their serial form.
        // The thread count MUST be explicit: with OMP_NUM_THREADS unset,
        // LAMMPS's "package omp 0" defaults to ONE thread, not all cores —
        // measured 29 ms/step vs 2.6 ms/step at 12 nm. Physical cores
        // (num_procs/2 on this hyperthreaded box) is the sweet spot.
#ifdef _OPENMP
        const int nthr = std::max(1, std::min(24, omp_get_num_procs() / 2));
#else
        const int nthr = 1;
#endif
        if (!lmpCmd("package omp %d neigh yes", nthr)) return false;
        if (!lmpCmd("suffix omp")) return false;
        // 'm': non-periodic with the region as MINIMUM bounds. 's' would
        // shrink-wrap a flat sheet to zero thickness and neighbor binning
        // dies with "box size << cutoff".
        if (!lmpCmd("boundary m m m")) return false;
        if (!lmpCmd("atom_style atomic")) return false;
        // Creation order must stay the storage order: ids[] then always maps
        // local index -> plugin index without a per-step sort surprise.
        if (!lmpCmd("atom_modify map array sort 0 0.0")) return false;
        if (!lmpCmd("region simbox block %.6g %.6g %.6g %.6g %.6g %.6g units box",
                    lo[0] - 25, hi[0] + 25, lo[1] - 25, hi[1] + 25, lo[2] - 15, zTop))
            return false;
        if (!lmpCmd("create_box %d simbox", mat->nsp)) return false;
        for (int s2 = 0; s2 < mat->nsp; s2++)
            if (!lmpCmd("mass %d %.6g", s2 + 1, mat->mass[s2])) return false;
        ...
        for (size_t i = 0; i < n; i++) {
            id[i] = (int32_t)i + 1;
            type[i] = tSpec[i] + 1;
            xyz[3 * i] = tx[i]; xyz[3 * i + 1] = ty[i]; xyz[3 * i + 2] = tz[i];
        }
        lammps_create_atoms(lmp, (int)n, id.data(), type.data(), xyz.data(),
                            nullptr, nullptr, 1);
        if (lammps_has_error(lmp) ||
            (long)lammps_get_natoms(lmp) != (long)n) {
            lmpError = "create_atoms failed";
            return false;
        }
        ...
        if (!lmpCmd("timestep %.10g", P.dtFs * 1e-3)) return false;
        if (!lmpCmd("fix mdint all nve")) return false;
        const double gv = viscousCoeff();
        if (gv > 0 && !lmpCmd("fix visc all viscous %.10g", gv)) return false;

        // Edge collar group from the as-built positions (matches isEdge).
        const double lim = P.Nnm * NM / 2 - 2 * NM;
        if (!lmpCmd("variable edg atom \"(abs(x)>%.6g)||(abs(y)>%.6g)\"", lim, lim))
            return false;
        if (!lmpCmd("group edge variable edg")) return false;
        if (P.edgeK >= 200) {
            if (!lmpCmd("fix pin edge setforce 0 0 0")) return false;
            if (!lmpCmd("velocity edge set 0 0 0")) return false;
        }

        if (!lmpCmd("fix ext all external pf/callback 1 1")) return false;
        lammps_set_fix_external_callback(lmp, "ext", (FixExternalFnPtr)&Instance::extCallback, this);
        if (!lmpCmd("fix_modify ext energy yes")) return false;

        // Settle the cut edges before dynamics: a cropped flake's under-
        // coordinated rim carries real bond-order strain, and released as
        // kinetic energy it reads as thousands of kelvin. Minimize, then
        // start cold. (The callback's substrate LJ and collar are included.)
        // CAPPED at 100 iterations: the point is settling the rim, not full
        // convergence — an uncapped minimize held the worker thread (and the
        // whole message queue behind it) for tens of seconds at 12 nm.
        ...
        ePot = lammps_get_thermo(lmp, "pe");  // energy visible before step 1
        return true;
    }
    ...
    void lmpStart() {
        ...
        if (lmpStartInner()) {
            ...
            std::string e = lmpError;
            lmpStop();
            lmpError = e;
            for (auto &c : lmpError)  // keep the notice JSON-safe
                if (c == '"' || c == '\\' || c == '\n' || c == '\r') c = ' ';
            say("{\"t\":\"notice\",\"msg\":\"LAMMPS engine failed: " + lmpError +
                "\"}");
            if (mat == &MATS[0]) {  // graphene can fall back to the toy model
                P.engine = "classic";
                rebuildConnectivity();
            }
        }
    ...
    // Re-issue the live settings a params message may have changed.
    void lmpApplyLive() {
        lmpDirty = false;
        lmpCmd("timestep %.10g", P.dtFs * 1e-3);
        lmpCmd("unfix visc");
        lammps_has_error(lmp);  // clear "no such fix" if it wasn't there
        lmpError.clear();
        const double gv = viscousCoeff();
        if (gv > 0) lmpCmd("fix visc all viscous %.10g", gv);
        lmpCmd("unfix pin");
        lammps_has_error(lmp);
        lmpError.clear();
        if (P.edgeK >= 200) {
            lmpCmd("fix pin edge setforce 0 0 0");
            lmpCmd("velocity edge set 0 0 0");
        }
        lmpPre = true;
    }

    bool lmpPre = true;  // next run needs a full pre-run force evaluation

    // Render bonds by distance for the LAMMPS engine: the potential owns the
    // physics, this list only feeds the bond lines and the strain colouring.
    void bondScanLmp() {
        const size_t n = nTop();
        std::vector<int32_t> bi, bj;
        const double cut = mat->bondCut, cut2 = cut * cut;
        // xy cell grid over the sheet
        double minx = 1e30, miny = 1e30, maxx = -1e30, maxy = -1e30;
        for (size_t i = 0; i < n; i++) {
            minx = std::min(minx, tx[i]); maxx = std::max(maxx, tx[i]);
            miny = std::min(miny, ty[i]); maxy = std::max(maxy, ty[i]);
        }
        const double cs = std::max(cut, 1.0);
        const int gx = (int)((maxx - minx) / cs) + 3, gy = (int)((maxy - miny) / cs) + 3;
        const double x0 = minx - cs, y0 = miny - cs;
        std::vector<std::vector<int32_t>> cells((size_t)gx * gy);
        for (size_t i = 0; i < n; i++) {
            int cxi = (int)((tx[i] - x0) / cs), cyi = (int)((ty[i] - y0) / cs);
            cxi = std::min(std::max(cxi, 0), gx - 1);
            cyi = std::min(std::max(cyi, 0), gy - 1);
            cells[(size_t)cyi * gx + cxi].push_back((int32_t)i);
        }
        for (size_t i = 0; i < n; i++) {
            const int cxi = std::min(std::max((int)((tx[i] - x0) / cs), 0), gx - 1);
            const int cyi = std::min(std::max((int)((ty[i] - y0) / cs), 0), gy - 1);
            for (int oy = -1; oy <= 1; oy++)
                for (int ox = -1; ox <= 1; ox++) {
                    const int ncx = cxi + ox, ncy = cyi + oy;
                    if (ncx < 0 || ncy < 0 || ncx >= gx || ncy >= gy) continue;
                    for (int32_t j : cells[(size_t)ncy * gx + ncx]) {
                        if (j <= (int32_t)i) continue;
                        // Bonds run metal<->non-metal (species 0 vs rest):
                        // this also keeps the two chalcogen planes, 3.1-3.6 Å
                        // apart, from being drawn as vertical X-X bonds.
                        if (mat->bondCross &&
                            !((tSpec[i] == 0) != (tSpec[j] == 0))) continue;
                        const double dx = tx[j] - tx[i], dy = ty[j] - ty[i], dz = tz[j] - tz[i];
                        if (dx * dx + dy * dy + dz * dz <= cut2) {
                            bi.push_back((int32_t)i); bj.push_back(j);
                        }
                    }
                }
        }
        if (bi != bondI || bj != bondJ) {
            bondI = std::move(bi);
            bondJ = std::move(bj);
            topoVersion++;
        }
    ...
    void lmpStep() {
        if (!lmpActive) {  // engine failed to start: hold the frame counter
            frame++; stepsDone++; stepsWindow++;
            return;
        }
        if (lmpDirty) lmpApplyLive();
        if (!lmpCmd("run 1 %s post no", lmpPre ? "pre yes" : "pre no")) {
            // A failed run would otherwise fail silently every step and look
            // like a frozen sheet. Kill the engine loudly instead.
            lmpActive = false;
            for (auto &c : lmpError)
                if (c == '"' || c == '\\' || c == '\n' || c == '\r') c = ' ';
            say("{\"t\":\"notice\",\"msg\":\"LAMMPS run failed: " + lmpError + "\"}");
            return;
        }
        lmpPre = false;
        ...
        if (v && type) {
            for (size_t i = 0; i < n; i++) {
                const double m = mat->mass[type[i] - 1];
                ke += 0.5 * m * (v[i][0] * v[i][0] + v[i][1] * v[i][1] + v[i][2] * v[i][2]);
            }
            ke *= AMU_A2_PS2_IN_EV;
        }
        if (v && type) {
            std::vector<double> s2(n), ms(n);
            for (size_t i = 0; i < n; i++) {
                s2[i] = v[i][0] * v[i][0] + v[i][1] * v[i][1] + v[i][2] * v[i][2];
                ms[i] = mat->mass[type[i] - 1];
            }
            setTemperatures(s2, ms, AMU_A2_PS2_IN_EV);
        } else {
            eKin = 0; temperature = temperatureAll = 0; nEvaporated = 0;
        }
    ...
    //   uint8  spec[n+m]    if flags & 8   (species per atom, word-padded)
    static const uint32_t F_BONDS = 1, F_SUB = 2, F_REG = 4, F_SPEC = 8;

    void packFrame() {
        const uint32_t n = (uint32_t)nTop(), m = (uint32_t)nSub();
        uint32_t flags = 0;
        if (topoVersion != sentTopoVersion) flags |= F_BONDS | F_SPEC;
        ...
        if (flags & F_SPEC) words += ((size_t)n + m + 3) / 4;
        ...
        if (flags & F_SPEC) {
            for (uint32_t i = 0; i < n; i++) *p++ = i < tSpec.size() ? tSpec[i] : 0;
            for (uint32_t i = 0; i < m; i++) *p++ = i < sSpec.size() ? sSpec[i] : 0;
            while (((size_t)(p - frameBuf.data())) % 4) *p++ = 0;
        }
    ...
    void sayState(long q) {
        ...
        snprintf(buf, sizeof buf,
                 ...
                 "\"temp\":%.4g,\"tempAll\":%.4g,\"nEvap\":%d,"
                 "\"gasN\":%.6g,\"gasV\":%.6g,\"gasP\":%.6g,\"gasR\":%.4g,"
                 "\"sps\":%.0f,\"ms\":%.3f,\"running\":%d,"
                 "\"elev\":%d,\"elevz\":%.3f,\"phase\":\"%s\","
                 "\"mat\":\"%s\",\"engine\":\"%s\",\"pot\":\"%s\",\"threads\":%d}",
                 q, (int)nTop(), (int)nSub(), (int)bondI.size(), frame, ePot, eKin,
                 ePot + eKin, temperature, temperatureAll, nEvaporated,
                 gasN, gasVA3, gasPa, gasRA,
                 statsClock > 0 ? stepsWindow / statsClock : 0.0, stepMs,
                 running ? 1 : 0, elevActive ? 1 : 0, elevz, elevPhase.c_str(),
                 mat->key,
                 useLmp() ? "lammps" : (P.engine == "ml" ? "ml" : "classic"),
                 useLmp() ? (lmpActive ? mat->potFile : "engine failed")
                          : (P.engine == "ml"
                                 ? (mlModel.empty() ? "ML sidecar" : mlModel.c_str())
                                 : "Morse toy model"),
#ifdef _OPENMP
                 omp_get_max_threads()
#else
    ...
    void readParams(const std::string &m) {
        ...
        P.gasT = num("gasT", P.gasT);
        P.gasGap = num("gasGap", P.gasGap);
        ...
        lmpDirty = true;     // timestep / damping / edge pin may have moved
    }

    // ------------------------------------------------- LAMMPS deck export

    // Continue-from-now snapshot -> a runnable LAMMPS input deck, pushed to
    // the UI one file per message for the browser to save. Engine-specific
    // only in where positions/velocities come from.
    void exportLmpDeck() {
        lmpexport::Deck D;
        D.lammpsEngine = useLmp() && lmpActive;
        D.materialKey = mat->key;
        D.nsp = mat->nsp;
        for (int i = 0; i < 3; i++) D.mass[i] = mat->mass[i];
        D.pairStyle = mat->pairStyle;
        D.potFile = mat->potFile;
        D.coeffTail = mat->coeffTail;
        {
            std::string pd = bundleDir() + "/potentials";
            for (auto &ch : pd) if (ch == '\\') ch = '/';
            D.potDir = pd;
        }
        D.dtFs = P.dtFs; D.gamma = P.gamma;
        D.sigma = P.sigma; D.eps = P.eps0 * P.escale;
        D.edgeK = P.edgeK;
        D.De = P.De; D.alpha = P.alpha; D.re = P.re;
        D.ktheta = P.ktheta; D.theta0 = P.theta0; D.kbend = P.kbend;
        D.maxV = P.maxV; D.maxDX = P.maxDX;
        D.breakMul = P.breakMul; D.reformMul = P.reformMul;
        D.viscous = viscousCoeff();
        D.Nnm = P.Nnm;
        D.profile = P.profile; D.protLoc = P.protLoc; D.elevMode = P.elevMode;
        D.Mxnm = P.Mxnm; D.Mynm = P.Mynm; D.Cxnm = P.Cxnm; D.Cynm = P.Cynm;
        D.bubbleRnm = P.bubbleRnm; D.bubbleP = P.bubbleP;
        D.betweenBoost = P.betweenBoost;
        D.liftRate = P.liftRate; D.targetDz = P.targetDz;
        D.holdSteps = P.holdSteps;
        D.elevActive = elevActive; D.elevz = elevz;
        D.henckyH0 = henckyH0(); D.areaAtom = mat->areaAtom;
        // fixed-N gas: the exporter needs the temperature, the peel-gap
        // threshold and the substrate contact plane to reproduce p = NkT/V
        // and the free peel front.
        D.gasT = P.gasT; D.gasGap = P.gasGap; D.z0ref = P.z0;
        D.epotNow = ePot; D.temperature = temperature; D.frame = frame;

        const size_t n = nTop();
        D.tSpec.assign(tSpec.begin(), tSpec.end());
        D.isEdge = isEdge;
        D.sx = sx; D.sy = sy; D.sz = sz;

        if (D.lammpsEngine) {
            // The truth lives inside LAMMPS: gather positions and velocities
            // (already in metal units, A and A/ps).
            std::vector<double> x(3 * n), v(3 * n);
            lammps_gather_atoms(lmp, (char *)"x", 1, 3, x.data());
            lammps_gather_atoms(lmp, (char *)"v", 1, 3, v.data());
            D.tx.resize(n); D.ty.resize(n); D.tz.resize(n);
            D.vx.resize(n); D.vy.resize(n); D.vz.resize(n);
            for (size_t i = 0; i < n; i++) {
                D.tx[i] = x[3 * i]; D.ty[i] = x[3 * i + 1]; D.tz[i] = x[3 * i + 2];
                D.vx[i] = v[3 * i]; D.vy[i] = v[3 * i + 1]; D.vz[i] = v[3 * i + 2];
            }
        } else {
            D.tx = tx; D.ty = ty; D.tz = tz;
            // classic velocities are A/fs; the data file wants A/ps
            D.vx.resize(n); D.vy.resize(n); D.vz.resize(n);
            for (size_t i = 0; i < n; i++) {
                D.vx[i] = vx[i] * 1000.0;
                D.vy[i] = vy[i] * 1000.0;
            ...
            D.bondI = bondI; D.bondJ = bondJ;
            D.angC = angC; D.angA = angA; D.angB = angB;
        }

        auto files = lmpexport::exportDeck(D);
        for (auto &f : files) {
            std::string esc;
            esc.reserve(f.text.size() + f.text.size() / 8);
            for (char c : f.text) {
                switch (c) {
                    case '"': esc += "\\\""; break;
                    case '\\': esc += "\\\\"; break;
            ...
            say("{\"t\":\"lmpexport\",\"name\":\"" + f.name + "\",\"text\":\"" + esc + "\"}");
        }
        say("{\"t\":\"lmpexport\",\"name\":\"__done__\",\"count\":" +
    ...
    void handle(const std::string &m) {
        ...
        if (t == "build") {
            readParams(m);
            // Material first: switching it re-bases the geometry defaults
            // (interlayer height, LJ) before any explicit values apply.
            const std::string wantMat = dexmsg::get_str(m, "material");
            const std::string wantEng = dexmsg::get_str(m, "engine");
            if (!wantEng.empty())
                P.engine = (wantEng == "lammps") ? "lammps"
                          : (wantEng == "ml") ? "ml" : "classic";
            if (!wantMat.empty() && wantMat != P.material) {
                P.material = matByKey(wantMat)->key;
                const MatSpec *nm = matByKey(P.material);
                P.z0 = nm->z0;
                P.sigma = nm->sigma;
                P.eps0 = nm->eps0;
                P.re = 1.42;  // classic engine is graphene-only anyway
            }
            ...
            if (useLmp()) {
                bondScanLmp();
                lmpStart();  // fresh instance = exact as-built state
            } else {
                computeForces();
            }
            sayState(q);
            ...
            if (on) {
                ...
                std::fill(vy.begin(), vy.end(), 0.0);
                std::fill(vz.begin(), vz.end(), 0.0);
                if (lmpActive) { lmpCmd("velocity all set 0.0 0.0 0.0"); lmpPre = true; }
                elevz = 0;
                elevActive = true;
                elevPhase = "up";
...
int advance(void *p, double dt) {
    ...
    // The budget was taken in full above, so whatever the 10 ms cap cut short
    // has to go BACK on the queue -- otherwise an explicit "step n" request is
    // silently truncated (n = 100 used to advance ~3 steps at 12 nm).
